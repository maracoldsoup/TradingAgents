...
# Load full KRX ticker→name map generated by scripts/build_krx_names.py.
# Manual entries defined above take priority over the generated file.
_krx_names_file = Path(__file__).parent / "dataflows" / "data" / "krx_ticker_names.json"
if _krx_names_file.exists():
    _full_names = json.loads(_krx_names_file.read_text(encoding="utf-8"))
    _full_names.update(DEFAULT_CONFIG["korean_ticker_names"])
    DEFAULT_CONFIG["korean_ticker_names"] = _full_names
